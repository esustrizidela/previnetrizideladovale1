...
$(function () {
  ...
  if (dt_adv_filter_table.length) {
    var dt_adv_filter = dt_adv_filter_table.DataTable({
      ajax: 'mod/prevbra/indic_5.php?qg=' + $('#qg').find(":selected").val(),
      ...
      columnDefs: [
        {
          render: function (data, type, row) {
            var resultado =  '<button type="button" class="btn btn-danger" onclick="opModal(\'#myModal\',\'mod/prevbra/vacinas.php?lig=' + row[20] + '&cid=' + row[21] + '&ind=5\');"><i class="fas fa-syringe" title="'+row[21]+'"></i></button>';
            if (data == 1){
              resultado =  '<button type="button" class="btn btn-success" onclick="opModal(\'#myModal\',\'mod/prevbra/vacinas.php?lig=' + row[20] + '&cid=' + row[21] + '&ind=5\');"><i class="fas fa-syringe" title="'+row[21]+'"></i></button>';
            } else {
              if (data == 2){
                resultado =  '<button type="button" class="btn btn-warning" onclick="opModal(\'#myModal\',\'mod/prevbra/vacinas.php?lig=' + row[20] + '&cid=' + row[21] + '&ind=5\');"><i class="fas fa-syringe" title="'+row[21]+'"></i></button>';
              }
            }
            return resultado;
          },
        ...
        {
          render: function (data, type, row) {
            var obs = row[28] + row[29] + '';
            var mobs = '';
            if (obs.length > 1){
              mobs = '&nbsp;<a href=\"#\" onclick="opModal(\'#myModal\',\'mod/prevbra/obs.php?lig=' + row[20] + '&cid=' + row[21] + '&ind=5\');"><i class=\"fas fa-info-circle\"></i></a>';
            }
            return data+'&nbsp;'+row[14]+'&nbsp;'+row[18]+mobs+'&nbsp;'+row[32];
          },
          targets: 13
        },
        {
        ...
        {
          render: function (data, type, row) {
            return row[33]+'&nbsp;'+data+'&nbsp;'+row[34]+'&nbsp;'+row[25]+'&nbsp;'+row[35]+'&nbsp;'+row[26]+'&nbsp;|&nbsp;'+row[27]+'&nbsp;'+row[31];
          },
          targets: 24
        },
        {
    ...
    $('#ind').on('change', function () {
      dt_adv_filter.columns(30).search($(this).find(":selected").val()).draw();
    });
  }
